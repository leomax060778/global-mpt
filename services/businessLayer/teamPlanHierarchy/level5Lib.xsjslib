...
var blLeadForm = mapper.getLeadForm();
...
var L5_MSG_OBJECT_NOT_FOUND = "The Object can not be found.";
...
var L5_MSG_ROUTE_NAME_NOT_FOUND = "The Route Name needs to be specified.";
...
function getHl5ByHl4Id(id, userId, includeLegacy, action, routeName) {
    if(!!action && action === "TREE_SELECTION"){
        if(!routeName){
            throw ErrorLib.getErrors().BadRequest("The Parameter routeName is not found", "", L5_MSG_ROUTE_NAME_NOT_FOUND);
        }

        dbUser.updateUserNavigationSelectedItem(userId, id, routeName);
    }
...
function getHl5ById(hl5Id, carryOver, isLegacy, fromCloneMethod, userId) {
    ...
    var budgetYearData = budgetYear.getBudgetYearByLevelParent(5, hl5.HL4_ID, true, isLegacy? 1:0);
    var budgetYearId = budgetYearData.BUDGET_YEAR_ID;
    ...
    if(!isLegacy) {
        ...
        if (!carryOver) {
            ...
            var targetAudience = getTargetAudienceByHl5Id(hl5Id);

            hl5.TARGET_AUDIENCE = targetAudience || [];
            ...
            hl5.CATEGORIES = getCategoryOption(hl5Id, fromCloneMethod);
            ...
            hl5.TOTAL_BUDGET = ((Number(hl5.BUDGET_EUROS) + Number(hl5.INTEL_TOTAL_BUDGET_EUROS)
                + Number(hl5.EXTERNAL_TOTAL_BUDGET_EUROS) + Number(hl5.SALE_TOTAL_EUROS)) * hl5.BUDGET_CURRENCY.VALUE);

            hl5.TOTAL_BUDGET_EUR = (Number(hl5.BUDGET_EUROS) + Number(hl5.INTEL_TOTAL_BUDGET_EUROS)
                + Number(hl5.EXTERNAL_TOTAL_BUDGET_EUROS) + Number(hl5.SALE_TOTAL_EUROS));
            ...
            hl5.TARGET_AUDIENCE = getTargetAudienceByHl5Id(hl5Id) || [];
            levelString = 'L6';
            levelId = hl5.HL5_ID;
        }
...
function insertHl5(data, userId) {
    ...
    if (data.STATUS_DETAIL_ID > 0) {
        ...
        if (hl5_id) {
            ...
            insertTargetAudience(hl5_id, data.TARGET_AUDIENCE, userId);
        }
        ...
        if (data.ACRONYM) {
            if(data.ENABLE_CRM_CREATION){
                ...
                return {SUCCESS_MESSAGE: MSG_ENABLE_CRM_CREATION, EXECUTE_CANCEL_CONFIRMATION: 1, HL5_ID: hl5_id};
            }
        } else {
    ...
    if (requestCategoryOption && requestCategoryOption.length) {
        ...
        requestCategoryOption.forEach(function (option) {
            ...
            if (option.FORM) {
                ...
                option.FORM.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID = option.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID;
                option.FORM.SERVICE_REQUEST_FORM_TYPE_ID = option.SERVICE_REQUEST_FORM_TYPE_ID;
                if(option.FORM.ORGANIZATION_TYPE && option.FORM.ORGANIZATION_TYPE === "REGIONAL"){
                	option.FORM.REGION_ID = option.FORM.ORGANIZATION_ID;
                }
                if(option.SERVICE_REQUEST_FORM_TYPE_ID){
                    switch (Number(option.SERVICE_REQUEST_FORM_TYPE_ID)) {
                        case SERVICE_REQUEST_FORM_TYPE.LEAD:
                            blLeadForm.insertLeadForm(option.FORM, userId);
                            break;
                        case SERVICE_REQUEST_FORM_TYPE.ACTIVITIES:

                            break;
                        case SERVICE_REQUEST_FORM_TYPE.SEGMENTATION:
...
function insertTargetAudience(hl5Id, targetAudience, userId){
    if (!hl5Id) {
        throw ErrorLib.getErrors().BadRequest("The Parameter HL5 ID is not found", "../insertTargetAudience", L5_MSG_INITIATIVE_NOT_FOUND);
    }
    if (!userId) {
        throw ErrorLib.getErrors().BadRequest("The Parameter User ID is not found", "../insertTargetAudience", L5_MSG_USER_NOT_FOUND);
    }

    hardDeleteTargetAudienceByHl5Id(hl5Id);
    ...
    if(targetAudience && targetAudience.length){
        ...
        dataHl5.insertTargetAudience(insertBulk);
    }
...
function hardDeleteTargetAudienceByHl5Id(hl5Id){
    if (!hl5Id) {
        throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "../deleteTargetAudienceByHl5Id", L5_MSG_INITIATIVE_NOT_FOUND);
    }
    return dataHl5.hardDeleteTargetAudienceByHl5Id(hl5Id);
}
...
function updateHl5RequestCategoryOption(hl5Id, requestCategoryOption, userId) {
    ...
    var optionSegmentationToDelete = [];
    var optionLeadToDelete = [];
    ...
    serviceRequestCategoryOptionList.forEach(function (option) {
        if (!requestCategoryOption.filter(function (elem) {
            return Number(elem.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID) === Number(option.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID);
        }).length) {
            option.HL5_ID = hl5Id;
            switch (Number(option.SERVICE_REQUEST_FORM_TYPE_ID)) {
                case SERVICE_REQUEST_FORM_TYPE.SEGMENTATION:
                    optionSegmentationToDelete.push({in_service_request_category_option_level_id: option.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID});
                    break;
                case SERVICE_REQUEST_FORM_TYPE.LEAD:
                    optionLeadToDelete.push({in_service_request_category_option_level_id: option.SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID});
                    break;
            }
        }
    });

    blSegmentation.deleteSegmentationFormByHl5IdOptionId(hl5Id, optionSegmentationToDelete, userId);

    blLeadForm.deleteLeadFormByHl5Id(hl5Id, userId);
...
function updateHl5(data, userId) {
    ...
    if (data.STATUS_DETAIL_ID > 0) {
        ...
        if(data.ENABLE_CRM_CREATION){
            ...
            } else if (!validationResult.crmFieldsHasChanged && Number(validationResult.statusId) === Number(HL5_STATUS.IN_CRM)) {
...
function isComplete(data, fromChangeStatusOnDemand) {
    ...
    if(!!data.CAMPAIGN_SUBTYPE_ID && requireEventDesType(data.CAMPAIGN_SUBTYPE_ID)){
        requiredFields.push("DES_TYPE_ID");
    }
...
function requireEventDesType(campaignSubTypeId){
    var desTypes = dataDesTypeCampaignSubType.getDesTypeCampaignSubType(campaignSubTypeId);

    return !!desTypes && !!desTypes.ASSIGNED && desTypes.ASSIGNED.length;
}
...
function validateBudgetStatus(hl5){
    var hl5_id = hl5.HL5_ID;
    var valid = true;
...
function isCategoryOptionComplete(data) {
    ...
    for (var i = 0; i < data.CATEGORIES.length; i++) {
        ...
        if (!data.HL5_ID && hl5Category.OPTIONS.length !== dataOption.getAllocationOptionCountByCategoryIdLevelId(hl5Category.CATEGORY_ID, LEVEL_STRING.toLowerCase(), data.HL4_ID)) {
...
function changeStatusOnDemand(hl5_id, userId, cancelConfirmation) {
    ...
    if (hl5.HL5_STATUS_DETAIL_ID != HL5_STATUS.IN_CRM
        ...
        if (statusId == HL5_STATUS.VALID_FOR_CRM || statusId == HL5_STATUS.CREATE_IN_CRM || statusId == HL5_STATUS.UPDATE_IN_CRM) {
            ...
            var targetAudience = getTargetAudienceByHl5Id(hl5_id);
...
function getServiceRequestCategoryOptionByHl5Id(hl5Id) {
    ...
    if (serviceRequestCategoryOptionList && serviceRequestCategoryOptionList.length) {
        var segmentationForms = [];
        var leadForms = [];
        var form = {};
        if(hl5Id){
            segmentationForms = blSegmentation.getSegmentationFormByHl5Id(hl5Id);
            leadForms = blLeadForm.getLeadFormByHl5Id(hl5Id);
        }
        ...
        for (var i = 0; i < serviceRequestCategoryOptionList.length; i++) {
            ...
            //Yes, we can do it shorter, but long strings can be confusing.

            if(segmentationForms[serviceRequestCategoryOptionList[i].SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID]){
                form = segmentationForms[serviceRequestCategoryOptionList[i].SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID];
            }else if(leadForms[serviceRequestCategoryOptionList[i].SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID]){
                form = leadForms[serviceRequestCategoryOptionList[i].SERVICE_REQUEST_CATEGORY_OPTION_LEVEL_ID];
            }
            ...
            result[serviceRequestCategoryOptionList[i].SERVICE_REQUEST_CATEGORY_ID].OPTIONS.push({
                ...
                SERVICE_REQUEST_FORM_TYPE_ID: serviceRequestCategoryOptionList[i].SERVICE_REQUEST_FORM_TYPE_ID,
                ...
                FORM: !!serviceRequestCategoryOptionList[i].IS_CHECKED ? form : undefined,
...
function crmFieldsHaveChanged(data, isComplete, userId, isNew, hl5Db) {
    ...
    if (!data.HL5_ID) {
        ...
        var oldHl5 = hl5Db ? hl5Db : dataHl5.getHL5VersionedById(data.HL5_ID);
        ...
        Object.keys(crmBindingFields).forEach(function (object) {
            crmBindingFields[object].forEach(function (field) {
                ...
                if (field == "PARENT_PATH") {
                    ...
                    parentPaths = pathBL.getPathByLevelParentForCrm(LEVEL_STRING.toLowerCase(), data.HL4_ID);
                }
                    ...
                    switch (field) {
                        ...
                        case "BUDGET":
                            ...
                            if (Number(data.STATUS_DETAIL_ID) !== Number(HL5_STATUS.IN_CRM)) {
                                fieldChanged = Number(oldHl5[field]).toFixed(2) !== Number(data[field]).toFixed(2);
                            } else {
                                fieldChanged = false;
                                budgetChanged = Number(oldHl5[field]).toFixed(2) !== Number(data[field]).toFixed(2);
                            }
                ...
                if (fieldChanged || pathChanged || (field === "BUDGET" && budgetChanged)) {
                    if (field === "PARENT_PATH") {
                        if (oldParentPath) {
                            if (pathChanged) {
                                pathBL.updParentPath(LEVEL_STRING.toLowerCase(), data.HL5_ID, parentPaths.parentPath, userId);
                            }
                        } else {
                    ...
                    var in_hl5_crm_binding_id = l5CrmBindigFields[field] || null;
...
function getSyndicatedSubProgramInformation() {
    ...
    result.SYNDICATED_SUB_PROGRAM_MAP = dataHl5.getSyndicatedSubProgramAssociationByHierarchyLevelId(HIERARCHY_LEVEL.HL5);
    ...
    if (data.CO_FUNDED && data.PARTNERS && data.PARTNERS.length) {
        ...
        data.PARTNERS.forEach(function (partner) {
            ...
            arrPartner.push({
                ...
                ,
                in_intel_project_id: PARTNER_TYPE.INTEL == partner.PARTNER_TYPE_ID && partner.INTEL_PROJECT_ID ? partner.INTEL_PROJECT_ID : null
                ,
...
/**
 * Validates if the L5 has Forecast at L5 selected. If it is the case, validates if the user added a comment.
 * If not, validates if there are KPIs, and if each one has the required attributes.
 * @deprecated
 * @param data (Object) complete payload that contains the TARGET_KPIS Object and FORECAST_AT_L5 flag
 * @returns {boolean}
 */
...
function getCategoryOption(hl5Id, fromCloneMethod) {
    return allocationCategoryOptionLevelLib.getHlCategoryOptionByLevelHlId(LEVEL_STRING, hl5Id, fromCloneMethod);
}
...
function getTargetAudienceByHl5Id(hl5Id) {
    if (!hl5Id) {
        throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "hl5Services/handleGet/getTargetAudienceByHl5Id", L5_MSG_INITIATIVE_NOT_FOUND);
    }
    var targetAudienceArray = dataHl5.getTargetAudienceByHl5Id(hl5Id);
    var result = [];
...
function clone(cloneHl5Id, userId) {
    var data = getHl5ById(cloneHl5Id, false, false, true).HL5;
    ...
    data = util.completeFromDynamicForm(data.HL4_ID, "L5", data, false, false, true);
...
function createFromEventRequest(eventRequest) {
    ...
    var budgetYear = dataValidation.getBudgetYearByIdLevel(eventRequest.HL4_ID, 'HL4')[0];
    eventRequest = completeCategories(eventRequest);
    newHl5.ENABLE_CRM_CREATION = budgetYear ? Number(budgetYear.ENABLE_CRM_CREATION) : 0;
    ...
    newHl5.AFFILIATED_WITH_LARGER_EVENT_ID = null;
    newHl5.AFFILIATED_EVENT_NAME = null;
    newHl5.EXISTING_CUSTOMER_PERCENTAGE = null;
    newHl5.GROSS_COST = null;
    newHl5.NET_COST = null;
    newHl5.EXPECTED_REVENUE = null;
    newHl5.EXPECTED_COFOUNDING = null;
    newHl5.REGISTRATION_PROCESS_ID = null;
    newHl5.EVENT_SUMMARY = null;
    newHl5.BUSINESS_CASE = null;
    newHl5.EVENT_FOLLOW_UP_ACTIVITIES = null;
    ...
    //create new L5
    //Now Target Audience (TARGET_AUDIENCE) is not supported from Event Request, but it should be after this
...
function updEnableCrmCreation(hl5Id, flag) {
    return dataHl5.updEnableCrmCreation(hl5Id, flag);
}

function updateHl5Budget(reqBody, userId){
    if(!reqBody){
        throw ErrorLib.getErrors().BadRequest("The Object is not found", "hl5Services/handlePut/updateHl5Budget", L5_MSG_OBJECT_NOT_FOUND);
    }

    if (!reqBody.HL5_ID){
        throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "hl5Services/handlePut/updateHl5Budget", L5_MSG_INITIATIVE_NOT_FOUND);
    }
    if (!userId){
        throw ErrorLib.getErrors().BadRequest("The Parameter User ID is not found", "hl5Services/handlePut/updateHl5Budget", L5_MSG_USER_NOT_FOUND);
    }

    return dataHl5.updateHl5Budget(reqBody, userId);
}
