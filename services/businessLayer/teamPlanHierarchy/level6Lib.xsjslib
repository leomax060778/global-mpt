...
var allocationCategoryOptionLevelLib = mapper
		.getAllocationCategoryOptionLevelLib();
...
var L6_CATEGORY_NOT_EMPTY = "Category cannot be empty.";
var L6_CATEGORY_INCORRECT_NUMBER = "Incorrect number of categories.";
...
var CREATED_FROM_EVENT_REQUEST_CRM_DESCRIPTION = util
		.getEventRequestCrmDescription();
...
var AFFILIATED_WITH_LARGER_EVENT_MAP = {
	    YES: 1,
	    NO: 2
};
...
function getHl6ByHl5Id(hl5Id, userId, includeLegacy, isLegacy) {
	...
	var hl5 = isLegacy ? dataHl5.getHl5LegacyById(hl5Id) : dataHl5
			.getHl5ById(hl5Id);
	var l4Id = hl5.HL4_ID;

	var totalBudget = hl5 ? hl5.BUDGET : 0;
	var totalAllocated = dataHl6.getHl6TotalBudgetByHl5Id(hl5Id,
			Number(isLegacy));
	var remainingBudget = totalBudget - totalAllocated;
	var isSuperAdmin = util.isSuperAdmin(userId);

	if (isLegacy) {
		hl6List = dataHl6.getHl6LegacyByHl5Id(hl5Id);
	} else {
		hl6List = dataHl6.getHl6ByHl5Id(hl5Id);
		enableCration = level5Lib.addChildPermission(hl5Id)
				&& level4Lib.addChildPermission(l4Id);
	}

	if (hl6List.length) {
		hl6List.forEach(function(hl6) {
			var aux = {};
			var actionPermission = getActionPermission(hl6, userId,
					isSuperAdmin);
	...
	response.budget_year = budgetYear.getBudgetYearByLevelParent(6, hl5Id,
			true, isLegacy ? 1 : 0);
	return response;
}

function getHl6ById(hl6Id, fromCloneMethod) {
    if (!hl6Id) {
        throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "", L6_MSG_INITIATIVE_NOT_FOUND);
    }

	var hl6 = JSON.parse(JSON.stringify(dataHl6.getHl6ById(hl6Id)));
	var isLegacy = !!hl6.HL5_LEGACY_ID;
	var budgetYearData = budgetYear.getBudgetYearByLevelParent(6, isLegacy? hl6.HL5_LEGACY_ID : hl6.HL5_ID, true, isLegacy? 1:0);
	var budgetYearId = budgetYearData.BUDGET_YEAR_ID; //Because it returns the object or the ID only depending on the legacy
	...
	if (Number(hl6.HL6_STATUS_DETAIL_ID) === HL6_STATUS.DELETED_IN_CRM) {
	...
	var targetAudience = getTargetAudienceByHl6Id(hl6Id);

	hl6.TARGET_AUDIENCE = targetAudience || [];
    ...
    hl6.BUDGET_EUROS = (Number(hl6.BUDGET));
    hl6.BUDGET_CURRENCY = {
    ...
    hl6.SALE_CURRENCY = {
        ID: internalCofunding.SALE_CURRENCY_ID,
        VALUE: internalCofunding.SALE_CURRENCY_VALUE
    };
    hl6.SALE_REQUESTS = internalCofunding.SALE_REQUESTS;

    hl6.CATEGORIES = getCategoryOption(hl6Id, fromCloneMethod);

    hl6.TOTAL_BUDGET = ((Number(hl6.BUDGET_EUROS)
        + Number(hl6.INTEL_TOTAL_BUDGET_EUROS)
        + Number(hl6.EXTERNAL_TOTAL_BUDGET_EUROS) + Number(hl6.SALE_TOTAL_EUROS)) * hl6.BUDGET_CURRENCY.VALUE);

    hl6.TOTAL_BUDGET_EUR = (Number(hl6.BUDGET_EUROS)
        + Number(hl6.INTEL_TOTAL_BUDGET_EUROS)
        + Number(hl6.EXTERNAL_TOTAL_BUDGET_EUROS) + Number(hl6.SALE_TOTAL_EUROS));
	...
	result.BUDGET_YEAR = budgetYearData;
...
function getLevel6ForSearch(userSessionID, budget_year_id, region_id,
		subregion_id, limit, offset) {
	var isSuperAdmin = util.isSuperAdmin(userSessionID);
	var list = dataHl6.getHl6ForSearch(userSessionID, isSuperAdmin ? 1 : 0,
			budget_year_id, region_id || 0, subregion_id || 0, limit,
			offset || 0);
	list = JSON.parse(JSON.stringify(list));
	list.result.forEach(function(elem) {
		elem.ENABLE_EDIT = util.getEnableEdit(elem.HL6_STATUS_DETAIL_ID,
				HL6_STATUS, userSessionID, isSuperAdmin);
	});
	return list;
}
...
function getHl6ByHl5IdUserId(hl5Id, userId) {
	...
	var isSuperAdmin = util.isSuperAdmin(userId);
	var hl6List = dataHl6.getHl6ByHl5IdUserId(hl5Id, userId, isSuperAdmin ? 1
			: 0);
	var result = {};
	var requestResult = {
		results : []
	...
	if (hl6List.length) {
		for (var i = 0; i < hl6List.length; i++) {
			...
			if (hl6List[i].HL6_ID) {
				var actionPermission = getActionPermission(hl6List[i], userId,
						isSuperAdmin);
				result[hl6List[i].HL5_ID].CHILDREN
						.push({
...
function getSyndicatedSubProgramInformation() {
	var result = {};
	result.SYNDICATED_SUB_PROGRAM_COLLECTION = dataHl5.getAllMarketingProgram();
	result.SYNDICATED_SUB_PROGRAM_MAP = dataHl5.getSyndicatedSubProgramAssociationByHierarchyLevelId(HIERARCHY_LEVEL.HL6);
...
function insertHl6(data, userId, isLegacy) {
	...
	if (!level4Lib.addChildPermission(l4Id)) {
		throw ErrorLib
				...
				.CustomError(
						"",
						"",
						"Cannot create Marketing Sub Tactic under this Program/Campaign. Program/Campaign is in Deleted In CRM status.");
	}
	...
	if (!isLegacy && !level5Lib.addChildPermission(data.HL5_ID)) {
		throw ErrorLib
				...
				.CustomError(
						"",
						"",
						"Cannot create Marketing Sub Tactic under this Marketing Tactic. Marketing Tactic is in Deleted In CRM status.");
	}
	...
	if (!hasAdditionalFields(data.CAMPAIGN_TYPE_ID)) {
		...
		data.AFFILIATED_WITH_LARGER_EVENT_ID = null;
		data.AFFILIATED_EVENT_NAME = null;
		data.EXISTING_CUSTOMER_PERCENTAGE = null;
		data.TARGET_AUDIENCE = null;
		data.GROSS_COST = null;
		data.NET_COST = null;
		data.EXPECTED_REVENUE = null;
		data.EXPECTED_COFOUNDING = null;
		data.REGISTRATION_PROCESS_ID = null;
		data.EVENT_SUMMARY = null;
		data.BUSINESS_CASE = null;
		data.EVENT_FOLLOW_UP_ACTIVITIES = null;
		data.EVENT_ANSWER_ID = null;
	}

	var automaticApproval = blLevel2
			.getHl2AllowAutomaticBudgetApprovalByHl4Id(l4Id);
	data.AUTOMATIC_APPROVAL = automaticApproval;
	var conversionValue = dataCurrency
			.getCurrencyValueId(data.EURO_CONVERSION_ID);
	...
	if (data.STATUS_DETAIL_ID > 0) {
		...
		try {
			...
			if (e.name == "CRM Constraint Error") {
				var error = ErrorLib.getErrors().AcronymError("", "",
						L6_MSG_INITIATIVE_CRM_ACRONYM);
				throw error;
			} else {
				throw e
			}
        ...
        if (hl6_id > 0) {
			...
			insertTargetAudience(hl6_id, data.TARGET_AUDIENCE, userId);
        }
        dataL6Report.updateLevel6ReportForDownload(hl6_id); //Update Processing// Report Export// Data
        if(data.ENABLE_CRM_CREATION){
            return hl6_id;
        } else {return {SUCCESS_MESSAGE: MSG_ENABLE_CRM_CREATION, EXECUTE_CANCEL_CONFIRMATION: 1, HL6_ID:hl6_id};
    }}
}
...
function validateHl6Upload(data) {
	...
	if (!data.ACRONYM) {
		var error = ErrorLib.getErrors().ImportError("",
				"level6Lib/validateHl6Upload-data.ACRONYM",
	...
	if (!data.ROUTE_TO_MARKET_ID) {
		var error = ErrorLib.getErrors().ImportError("",
				"level6Lib/validateHl6Upload-data.ROUTE_TO_MARKET_ID",
				L6_MSG_ROUTE_TO_MARKET_NULL);
		error.row = valuesToArray(data);
		throw error;
	}
	...
	if (existsHl6(data)) {
		var error = ErrorLib.getErrors().ImportError("",
				"level6Lib/validateHl6Upload-existsHl6",
				L6_MSG_INITIATIVE_CRM_ACRONYM_EXISTS);
		error.row = valuesToArray(data);
		throw error;
	}
...
function insertHl6FromUpload(data, userId) {
	...
	if (validateHl6Upload(data)) {
         ...
         */
        var budgetYear = dataValidation.getBudgetYearByIdLevel(data.HL5_ID, 'HL5')[0];
		...
		if (hl6_id) {
			...
			regions.forEach(function(myBudget) {
				...
				dataHl6.insertHl6BudgetSalesUpload(myBudget.HL6_ID,
						myBudget.REGION_ID, 0, ORGANIZATION_TYPE["REGIONAL"],
						"", userId, data.EURO_CONVERSION_ID);
			});

			centralTeams.forEach(function(sale) {
				sale.HL6_ID = hl6_id;
				dataHl6.insertHl6BudgetSalesUpload(sale.HL6_ID, sale.HL2_ID, 0,
						ORGANIZATION_TYPE["CENTRAL"], "", userId,
...
function insertTargetAudience(hl6Id, targetAudience, userId){
	if (!hl6Id) {
		throw ErrorLib.getErrors().BadRequest("The Parameter HL6 ID is not found", "../insertTargetAudience", L6_MSG_INITIATIVE_NOT_FOUND);
	}
	if (!userId) {
		throw ErrorLib.getErrors().BadRequest("The Parameter User ID is not found", "../insertTargetAudience", L6_MSG_USER_NOT_FOUND);
	}

	hardDeleteTargetAudienceByHl6Id(hl6Id);

	var insertBulk = [];

	if(targetAudience && targetAudience.length){
		if (!Array.isArray(targetAudience)) {
			targetAudience = targetAudience.split(",");
		}
		targetAudience.forEach(function(target){
			insertBulk.push({
				in_hl6_id: hl6Id,
				in_target_audience_id: Number(target),
				in_created_user_id: userId
			});
		});

		dataHl6.insertTargetAudience(insertBulk);
	}
...
function hardDeleteTargetAudienceByHl6Id(hl6Id){
	if (!hl6Id) {
		throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "../deleteTargetAudienceByHl6Id", L6_MSG_INITIATIVE_NOT_FOUND);
	}
	return dataHl6.hardDeleteTargetAudienceByHl6Id(hl6Id);
}

function insertInCrmBinding(crmBindingChangedFields,
		crmBindingChangedFieldsUpdate, hl6Id) {
	if (hl6Id) {
...
function updateHl6(data, userId, isLegacy) {
	if (!data.HL6_ID)
		throw ErrorLib.getErrors()
				.CustomError("", "hl6Services/handlePost/updatehl6",
						L6_MSG_INITIATIVE_NOT_FOUND);
	...
	var hl5 = isLegacy ? dataHl5.getHl5LegacyById(data.HL5_LEGACY_ID) : dataHl5
			.getHl5ById(data.HL5_ID);
	var hl5Id = data.HL5_ID || data.HL5_LEGACY_ID;
	var l4Id = data.HL4_ID || hl5.HL4_ID;
	...
	if (!level4Lib.addChildPermission(l4Id)) {
		throw ErrorLib
				...
				.CustomError(
						"",
						"",
						"Cannot update this selected Marketing Sub Tactic under this Program/Campaign. Program/Campaign is in Deleted In CRM status.");
	}
	...
	if (!isLegacy && !level5Lib.addChildPermission(data.HL5_ID)) {
		throw ErrorLib
				...
				.CustomError(
						"",
						"",
						"Cannot update this selected Marketing Sub Tactic under this Marketing Tactic. Marketing Tactic is in Deleted In CRM status.");
	}
	...
	if (hl6StatusId === HL6_STATUS.DELETED_IN_CRM) {
		throw ErrorLib
				...
				.CustomError(
						"",
						"",
						"Cannot update this selected Marketing Sub-Tactic, because the status doesn´t allow it.");
	}

	// TODO: Super admin validation added because of SAP new requirements,
	// refactor this
	if (!util.isSuperAdmin(userId)) {
		if (hl6StatusId === HL6_STATUS.CREATE_IN_CRM
				|| hl6StatusId === HL6_STATUS.UPDATE_IN_CRM) {
			throw ErrorLib
					.getErrors()
					.CustomError(
							"",
							"",
							"Cannot update this selected Initiative/Campaign, because the status doesn´t allow it.");
		}
	}
	...
	var ownMoneyBudgetSpendRequestStatus = budgetSpendRequest
			.getOwnMoneyBudgetSpendRequestStatusByHlIdLevel(data.HL6_ID,
					LEVEL_STRING);
	if (!ownMoneyBudgetSpendRequestStatus) {
		budgetSpendRequest.insertOwnMoneyBudgetSpendRequest(data.BUDGET,
				data.HL6_ID, LEVEL_STRING, userId, !!data.ALLOW_BUDGET_ZERO
						|| (automaticApproval && data.IN_BUDGET));
	} else {
		var budgetSpendRequestStatus = budgetSpendRequest
				.getBudgetSpendRequestsStatus();

		if ((Number(objHL6.BUDGET).toFixed(2) !== Number(data.BUDGET)
				.toFixed(2))
				|| (ownMoneyBudgetSpendRequestStatus != budgetSpendRequestStatus.APPROVED)) {

			budgetSpendRequest.updateOwnMoneyBudgetSpendRequestByHlIdLevel(
					data.HL6_ID, LEVEL_STRING, data.BUDGET,
					!!data.ALLOW_BUDGET_ZERO
							|| (automaticApproval && data.IN_BUDGET), userId);
		}
	}

    if (data.ALLOW_BUDGET_ZERO) {
        budgetSpendRequest.setBudgetSpendRequestStatusNoLongerRequested(
            data.HL6_ID, LEVEL_STRING, userId);
    } else if (!data.CO_FUNDED) {
        budgetSpendRequest.disableCoFundedBudgetSpendRequests(data.HL6_ID,
            LEVEL_STRING, userId);
    } else {
	...
	if (data.STATUS_DETAIL_ID > 0) {
        ...
        try {
            dataHl6.updateHl6(
				...
				data.AFFILIATED_WITH_LARGER_EVENT_ID,
				data.AFFILIATED_EVENT_NAME,
				data.EXISTING_CUSTOMER_PERCENTAGE,
				data.GROSS_COST,
				data.NET_COST,
				data.EXPECTED_REVENUE,
				data.EXPECTED_COFOUNDING,
				data.REGISTRATION_PROCESS_ID,
				data.EVENT_SUMMARY,
				data.BUSINESS_CASE,
				data.EVENT_FOLLOW_UP_ACTIVITIES,
                data.CITY,
                data.COUNTRY_ID || null,
                data.RESULTS_CAMPAIGN_Q1 || 0,
                data.RESULTS_CAMPAIGN_Q2 || 0,
                ...
                data.ENABLE_CRM_CREATION,
				data.EVENT_ANSWER_ID
            );
        } catch (e) {
            if (e.name == "CRM Constraint Error") {
		...
		if (data.STATUS_DETAIL_ID != objHL6.HL6_STATUS_DETAIL_ID) {
			dataHl6.insertHl6LogStatus(data.HL6_ID,
					objHL6.HL6_STATUS_DETAIL_ID, userId);
			dataHl6.updateDeletionReason(data.HL6_ID, null, userId);
		}
		data.HL2_ID = dataHl2.getHl2ByHl4Id(l4Id).HL2_ID;

		// Check if the KPIs were changed and update them
		var originalKPIs = expectedOutcomesLib.getExpectedOutcomesByHl6Id(
				data.HL6_ID, hl5Id);
		var changedKPIs = validateChangedKPIs(data, originalKPIs,
				objHL6.FORECAST_AT_L5);
		updateExpectedOutcomes(data, userId);

		updateBudgetDistribution(data, userId);
		updateCategoryOption(data, userId);

		insertTargetAudience(data.HL6_ID, data.TARGET_AUDIENCE, userId);
        ...
        } else if (!validationResult.crmFieldsHasChanged && Number(validationResult.statusId) === Number(HL6_STATUS.IN_CRM)) {
			return {SUCCESS_MESSAGE: "Your Tactic was successfully saved. The status will stay In CRM."};
		} else {
            if (!data.ACRONYM) {
        ...
        return data;
            ...
            return {SUCCESS_MESSAGE: MSG_ENABLE_CRM_CREATION, EXECUTE_CANCEL_CONFIRMATION: 1, HL6_ID: data.HL6_ID};
        }
    } else {
...
function validateChangedKPIs(hl6Object, oldKPIs, oldForecastAtL5) {
	...
	if (!!oldForecastAtL5 !== !!hl6Object.FORECAST_AT_L5
		...
		hl6Object.TARGET_KPIS.KPIS
				.forEach(function(kpi) {
					currentOldKPI = oldKPIs.KPIS
							.find(function(oldKpi) {
					...
					if (!currentOldKPI) {
						...
						changed = ((kpi.OUTCOMES_TYPE_NAME !== currentOldKPI.OUTCOMES_TYPE_NAME)
								|| (kpi.OUTCOMES_NAME !== currentOldKPI.OUTCOMES_NAME)
								|| (Number(kpi.OUTCOMES_TYPE_ID) !== Number(currentOldKPI.OUTCOMES_TYPE_ID))
								|| (Number(kpi.EURO_VALUE) !== Number(currentOldKPI.EURO_VALUE)) || (Number(kpi.VOLUME_VALUE) !== Number(currentOldKPI.VOLUME_VALUE)));
					}
				});
	}
...
function hasAdditionalFields(campaignTypeId) {
	return campaignTypeId
			&& dataCampaignType.getCampaignTypeById(campaignTypeId).SHOW_ADDITIONAL_FIELDS
}

function findHLSalesId(Sales, OrganizationId, OrganizationType) {
	var hlSaleId = null;
	for (var i = 0; i < Sales.length; i++) {
		if (Sales[i].ORGANIZATION_ID == OrganizationId
				&& Sales[i].ORGANIZATION_TYPE == OrganizationType) {
...
function deleteHl6(hl6, userId, rollBack, isCascadeDeletion) {
	...
	var hl6StatusId = !rollBack ? Number(dataHl6.getHl6StatusByHl6Id(hl6Id).HL6_STATUS_DETAIL_ID)
			: 0;

	if (hl6StatusId !== HL6_STATUS.DELETED_IN_CRM
			&& hl6StatusId !== HL6_STATUS.DELETION_REQUEST) {
		if (!rollBack
				&& isCascadeDeletion
				&& (hl6StatusId === HL6_STATUS.IN_CRM
						|| hl6StatusId === HL6_STATUS.CREATE_IN_CRM || hl6StatusId === HL6_STATUS.UPDATE_IN_CRM)) {
			throw ErrorLib.getErrors().CustomError("", "",
					L6_MSG_CANNOT_DEL_STATUS);
		}

		if (!rollBack && !isCascadeDeletion
				&& hl6StatusId === HL6_STATUS.IN_CRM) {
			if (hl6.DELETEION_REASON && hl6.DELETEION_REASON.trim()) {
				dataHl6.updateDeletionReason(hl6Id, hl6.DELETEION_REASON,
						userId);
			}
			...
			dataCategoryOptionLevel.deleteCategoryOption(hl6Id, userId,
					LEVEL_STRING);
			dataCategoryOptionLevel.deleteCountryCategoryOption(hl6Id, userId,
					LEVEL_STRING);

			dataHl6.delHl6Budget(hl6Id, userId);

			// delete HL6_SALE_BUDGET_SPEND_REQUEST
			databudgetSpendRequest.delAllHlSaleBudgetSpendRequestByHlId(hl6Id,
					userId, LEVEL_STRING); // ready
			dataHl6.delHl6Sale(hl6Id, userId);
			// BUDGET_SPEND_REQUEST_LOG_STATUS
			databudgetSpendRequest.delAllBudgetSpendRequestLogStatusByHlId(
					hl6Id, userId, LEVEL_STRING);// ready
			// BUDGET_SPEND_REQUEST_MESSAGE
			databudgetSpendRequest.delAllBudgetSpendRequestMessageByHlId(hl6Id,
					userId, LEVEL_STRING);
			// DEL_ALL_BUDGET_SPEND_REQUEST_BY_HL6_ID
			databudgetSpendRequest.delAllBudgetSpendRequestByHlId(hl6Id,
					userId, LEVEL_STRING);// ready
...
function isComplete(data, fromChangeStatusOnDemand) {
	...
	//Required fields that are not in Processing Report
	var requiredFields = [];

	if(!!data.SHOW_ADDITIONAL_FIELDS && Number(data.EVENT_ANSWER_ID) !== 3){
		requiredFields = [
			"EVENT_OWNER",
			"COUNTRY_ID",
			"CITY",
		...
		if(!!data.AFFILIATED_WITH_LARGER_EVENT_ID && Number(data.AFFILIATED_WITH_LARGER_EVENT_ID) === AFFILIATED_WITH_LARGER_EVENT_MAP.YES){
			requiredFields.push("AFFILIATED_EVENT_NAME");
		}
	}
	...
	for (var i = 0; i < crmBindingFields.length; i++) {
		...
		case 'CATEGORY':
			isComplete = !data.HL6_ID ? data.CATEGORIES.length === dataCategory
					.getAllocationCategoryCountByHlId(LEVEL_STRING
							.toLowerCase()) : isComplete;
			...
			if (isComplete) {
				for (var j = 0; j < data.CATEGORIES.length; j++) {
					...
					isComplete = isComplete
							&& !!(hl6Category.CATEGORY_ID && Number(hl6Category.CATEGORY_ID));

                    if(hl6Category.HIDDEN){
                    	if(hl6Category.MAKE_CATEGORY_MANDATORY){
                            isComplete = isComplete && hl6Category.OPTIONS.length === 1;
                        }
                    } else {
                        var countOptions = dataOption
                            .getAllocationOptionCountByCategoryIdLevelId(
                                    hl6Category.CATEGORY_ID, LEVEL_STRING
                                            .toLowerCase());
						isComplete = isComplete
								&& (!data.HL6_ID ? hl6Category.OPTIONS.length === countOptions
										: isComplete);
					}
                    ...
                    hl6Category.OPTIONS
							.forEach(function(option) {
								...
								isComplete = isComplete
										&& !!(option.OPTION_ID && Number(option.OPTION_ID));
								isComplete = isComplete
										&& !(Number(option.AMOUNT) > 100 || Number(option.AMOUNT) < 0); // Budget%
								isComplete = isComplete
										&& !(Number(option.AMOUNT_KPI) > 100 || Number(option.AMOUNT_KPI) < 0); // KPI%

								percentagePerOption = percentagePerOption
										+ Number(option.AMOUNT); // Budget%
								percentagePerOptionKpi = percentagePerOptionKpi
										+ Number(option.AMOUNT_KPI); // KPI%
							});

					isComplete = isComplete
							&& percentagePerOption === 100
							|| (!hl6Category.MAKE_CATEGORY_MANDATORY && percentagePerOption === 0); // Budget%

					isComplete = isComplete
							&& percentagePerOptionKpi === 100
							|| (!hl6Category.MAKE_CATEGORY_MANDATORY && percentagePerOptionKpi === 0); // KPI%

					var totalOptionsSelected = hl6Category.OPTIONS
							.filter(function(option) {
								return (option.AMOUNT
										&& Number(option.AMOUNT) !== 0 || option.AMOUNT_KPI
										&& Number(option.AMOUNT_KPI) !== 0);
							}).length;

					isComplete = isComplete
							&& totalOptionsSelected <= hl6Category.OPTIONS_LIMIT;
					if (!isComplete) {
						break;
					}
				}
		...
		case "HL6_CRM_DESCRIPTION":
			isComplete = fromChangeStatusOnDemand ? !!data.HL6_CRM_DESCRIPTION
	...
	if (isComplete) {
		isComplete = Number(data.ALLOW_BUDGET_ZERO)
				|| ((Number(data.BUDGET_SPEND_Q1) || 0)
		...
		if (!Number(data.FORECAST_AT_L5)) {
			isComplete = isComplete
					&& !!((data.TARGET_KPIS && data.TARGET_KPIS.KPIS && data.TARGET_KPIS.KPIS.length) || data.TARGET_KPIS.COMMENTS
							.trim());
		}
	}
	return isComplete;
	...
	if (Number(data.ALLOW_BUDGET_ZERO)) {
		...
		if (data.AUTOMATIC_APPROVAL) {
			...
			if (!!Number(data.BUDGET)) {
				...
				var hasBudgetRequestApproved = !!Number(budgetSpendRequest
						.countApprovedBudgetRequestByHl6Id(data.HL6_ID || '0'));
				var hasBudgetRequestPending = !Number(budgetSpendRequest
						.countPendingBudgetRequestByHl6Id(data.HL6_ID || '0'));
...
// The Marketing Sub Tactic actual start date and actual end date cannot be
// outside of the parent L5 date range.
function validateDateRange(data, hl5) {
	if (hl5.ACTUAL_START_DATE && hl5.ACTUAL_END_DATE) {
		if (!data.ACTUAL_START_DATE)
			throw ErrorLib.getErrors().CustomError("", "",
					L6_MSG_INITIATIVE_ACTUAL_START_DATE);

		if (!data.ACTUAL_END_DATE)
			throw ErrorLib.getErrors().CustomError("", "",
					L6_MSG_INITIATIVE_ACTUAL_END_DATE);

		if (new Date(hl5.ACTUAL_START_DATE) > new Date(data.ACTUAL_START_DATE)
				|| new Date(data.ACTUAL_END_DATE) > new Date(
						hl5.ACTUAL_END_DATE))
			throw ErrorLib.getErrors().CustomError("", "",
					L6_MSG_INITIATIVE_ACTUAL_START_END_DATE_PARENT_RANGE);

		if (!!hl5.PLANNED_START_DATE
				&& !!hl5.PLANNED_END_DATE
				&& (new Date(hl5.PLANNED_START_DATE) > new Date(
						data.PLANNED_START_DATE) || new Date(
						data.PLANNED_END_DATE) > new Date(hl5.PLANNED_END_DATE)))
			throw ErrorLib.getErrors().CustomError("", "",
					L6_MSG_INITIATIVE_PLANNED_START_END_DATE_PARENT_RANGE);
	}
	return true;
}
...
function validateHl6(data, userId, hl5) {
	...
	if (!Number(data.ALLOW_BUDGET_ZERO)
			...
			&& (!data.CO_FUNDED || data.PARTNERS.length == 0
					&& data.SALE_REQUESTS.length == 0)) {
		throw ErrorLib.getErrors().CustomError("", "",
				L6_MSG_REQUIRE_SPEND_BUDGET_VALIDATION);
	}
	...
	if (data.HL6_ID) {
		...
		if (existInCrm && hl6.ACRONYM != data.ACRONYM)
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_PROPERTIES_CANNOT_UPDATE);
	}
	...
	if (isHl6Complete) {
		...
		if (!data.CRM_DESCRIPTION || !data.CRM_DESCRIPTION.trim()
				|| (data.CRM_DESCRIPTION.trim()).length > 40) {
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_CRM_DESCRIPTION)
		}

		if (!Number(data.DISTRIBUTION_CHANNEL_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_DISTRIBUTION_CHANNEL);

		if (!data.COST_CENTER_ID || data.COST_CENTER_ID < 0)
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_COST_CENTER_NOT_VALID);

		if (!data.EMPLOYEE_RESPONSIBLE_USER
				|| !data.EMPLOYEE_RESPONSIBLE_USER.trim())
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_RESPONSIBLE_NOT_VALID);

		if (!data.ROUTE_TO_MARKET_ID || !Number(data.ROUTE_TO_MARKET_ID)
				|| !dataRTM.getRouteToMarketById(data.ROUTE_TO_MARKET_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_ROUTE_TO_MARKET);

		if (!data.CAMPAIGN_OBJECTIVE_ID || !Number(data.CAMPAIGN_OBJECTIVE_ID)
				|| !dataObj.getObjectiveById(data.CAMPAIGN_OBJECTIVE_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_CAMPAIGN_OBJECTIVE);

		if (!data.CAMPAIGN_TYPE_ID || !Number(data.CAMPAIGN_TYPE_ID)
				|| !dataCT.getCampaignTypeById(data.CAMPAIGN_TYPE_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_CAMPAIGN_TYPE);

		if (!data.CAMPAIGN_SUBTYPE_ID || !Number(data.CAMPAIGN_SUBTYPE_ID)
				|| !dataCST.getCampaignSubTypeById(data.CAMPAIGN_SUBTYPE_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_CAMPAIGN_SUBTYPE);

		if (!data.PRIORITY_ID || data.PRIORITY_ID < 0) {
			throw ErrorLib.getErrors().CustomError("", "",
					L6_PRIORITY_NOT_VALID);
		}

		if (!data.BUSINESS_OWNER_ID || data.BUSINESS_OWNER_ID < 0) {
			throw ErrorLib.getErrors().CustomError("", "",
					L6_BUSINESS_OWNER_NOT_VALID);
		}

		if (!data.SALES_ORGANIZATION_ID
				|| !Number(data.SALES_ORGANIZATION_ID)
				|| !dataMO
						.getMarketingOrganizationById(data.SALES_ORGANIZATION_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_SALES_ORGANIZATION);
		...
		if (util.validateDateEndMayorStart((new Date(data.ACTUAL_START_DATE)),
				(new Date(data.ACTUAL_END_DATE))))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_INVALID_DATE_RANGE);

		if (!Number(data.EURO_CONVERSION_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_CURRENCY);

		if (!data.BUDGET_SPEND_Q1 && !data.BUDGET_SPEND_Q2
				&& !data.BUDGET_SPEND_Q3 && !data.BUDGET_SPEND_Q4)
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_BUDGET_SPEND);

		if (!Number(data.ALLOW_BUDGET_ZERO)
				&& ((Number(data.BUDGET_SPEND_Q1) || 0)
						+ (Number(data.BUDGET_SPEND_Q2) || 0)
						+ (Number(data.BUDGET_SPEND_Q3) || 0) + (Number(data.BUDGET_SPEND_Q4) || 0)) < 100)
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_MSG_INITIATIVE_BUDGET_SPEND_PERCENT);
		...
		var crmFieldsHasChangedResult = crmFieldsHaveChanged(data,
				data.ALLOW_BUDGET_ZERO
						|| (categoryOptionComplete && myBudgetComplete), userId);
		...
		if (data.HL6_ID) {
			if (categoryHasChanged) {
				...
				insertInCrmBinding(field, [], data.HL6_ID); // Inset in
															// HL6_CRM_BINDING a
															// row for
															// categoryChange
			}
            if (!crmFieldsHasChanged && !categoryHasChanged
                && budgetChanged
                && Number(hl6.HL6_STATUS_DETAIL_ID) == HL6_STATUS.IN_CRM) {
...
function validateBudgetStatus(hl6){
	var hl6_id = hl6.HL6_ID;
	var valid = true;
	if (!hl6.ALLOW_BUDGET_ZERO) {
        ...
        if ((!ownMoneyBudgetSpendRequestStatus || (!!ownMoneyBudgetSpendRequestStatus && Number(ownMoneyBudgetSpendRequestStatus) !== budgetSpendRequestStatus.APPROVED))
            || budgetSpendRequest.checkPendingCoFundedBudgetSpendRequestByHlIdLevel(hl6_id, LEVEL_STRING)
        ) {
            valid = false;
        }
...
function categoryChanged(data, existInCrm, userId) {
	...
	optionChange = CompareCategories(data.CATEGORIES, hl6_categoryBD,
			existInCrm, data.HL6_ID, hl6_categoryBD.length);

	return optionChange;
}

function CompareOptions(Option1, Option2, existInCrm,
		hasCategoryOptionInCrmVersion) {
...
function CompareListOptions(ListOption1, ListOption2, existInCrm,
	...
	for (var i = 0; i < ListOption1.length; i++) {
		...
		flag = CompareOptions(option, getOptionFromList(ListOption2,
				option.OPTION_ID), existInCrm, hasCategoryOptionInCrmVersion)
				|| flag;
	}
	return flag;
}
...
function CompareCategoryOption(Category1, Category1_id, ListCategories,
		existInCrm, hasCategoryOptionInCrmVersion) {
	var Category2 = hasCategoryOptionInCrmVersion ? getCategoryFromList(
			ListCategories, Category1_id) : {
		OPTIONS : []
	};
...
function CompareCategories(ListCategories1, ListCategories2, existInCrm, hl6Id,
		hasCategoryOptionInCrmVersion) {

	var flag = false;
	var actualCategory = util
			.getCategoryById(LEVEL_STRING.toLowerCase(), hl6Id);
	for (var i = 0; i < ListCategories1.length; i++) {
		...
		if (actualCategory[category.CATEGORY_ID].IN_PROCESSING_REPORT) {
			flag = CompareCategoryOption(category, category.CATEGORY_ID,
					ListCategories2, existInCrm, hasCategoryOptionInCrmVersion)
					|| flag;
		}
...
function isMyBudgetComplete(budgetDistribution) {
	...
	budgetDistribution.forEach(function(budgetDistribution) {
		if (!budgetDistribution.ORGANIZATION_ID
				|| !Number(budgetDistribution.ORGANIZATION_ID))
			throw ErrorLib.getErrors().CustomError(
					"",
					"",
					levelCampaign + " in My Budget "
							+ budgetDistribution.ORGANIZATION_ID
							+ " can not be found.");
	...
	if (myBudgetTotalPercentage > 100)
		throw ErrorLib.getErrors().CustomError("", "",
				L6_MSG_INITIATIVE_BUDGET_PERCENT);

	myBudgetTotalPercentage = myBudgetTotalPercentage < 100 ? 0
			: myBudgetTotalPercentage;
...
function isCategoryOptionComplete(data) {
	...
	for (var i = 0; i < data.CATEGORIES.length; i++) {
		...
		if (!hl6Category.CATEGORY_ID || !Number(hl6Category.CATEGORY_ID))
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6", L6_CATEGORY_NOT_VALID);
        ...
        if(!data.HL6_ID && !hl6Category.HIDDEN && hl6Category.OPTIONS.length !== dataOption
            .getAllocationOptionCountByCategoryIdLevelId(hl6Category.CATEGORY_ID, LEVEL_STRING.toLowerCase(), data.HL4_ID)){
        	throw ErrorLib.getErrors().CustomError("", "", L6_CATEGORY_OPTIONS_INCORRECT_NUMBER);
        }
		...
		hl6Category.OPTIONS
				.forEach(function(option) {
					if (!option.OPTION_ID || !Number(option.OPTION_ID))
						throw ErrorLib.getErrors().CustomError("", "",
								L6_CATEGORY_OPTION_NOT_VALID);
					if ((parseFloat(option.AMOUNT) && !Number(option.AMOUNT))
							|| Number(option.AMOUNT) > 100
							|| Number(option.AMOUNT) < 0)
						throw ErrorLib.getErrors().CustomError(
								"",
								"",
								"Option value is not valid (actual value "
										+ option.AMOUNT + ")");
					if ((parseFloat(option.AMOUNT_KPI) && !Number(option.AMOUNT_KPI))
							|| Number(option.AMOUNT_KPI) > 100
							|| Number(option.AMOUNT_KPI) < 0)
						throw ErrorLib.getErrors().CustomError(
								"",
								"",
								"Option KPI value is not valid (actual value "
										+ option.AMOUNT_KPI + ")");

					percentagePerOption = percentagePerOption
							+ Number(option.AMOUNT);
					percentagePerOptionKpi = percentagePerOptionKpi
							+ Number(option.AMOUNT_KPI);
				});
		if (percentagePerOption > 100) {
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_CATEGORY_TOTAL_PERCENTAGE);
		}
...
function checkBudgetStatus(objHl5, hl6_id, new_hl6_budget, hl5, isLegacy) {
	...
	if (Number(objHl5) && (new_hl6_budget || new_hl6_budget == 0)) {
		...
		objHl.HL5_ID = objHl5;
		objHl.HL6_ID = hl6_id;
		var hl5AllocatedBudget = isLegacy ? dataHl5
				.getHl5LegacyAllocatedBudget(objHl.HL5_ID, hl6_id) : dataHl5
				.getHl5AllocatedBudget(objHl.HL5_ID, hl6_id);
		return (Number(hl5.BUDGET) - Number(hl5AllocatedBudget) - Number(new_hl6_budget)) >= 0 ? 1
				: 0;
	} else {
		...
		if (resultHl6.length > 0) {
			...
			for (var i = 0; i < resultHl6.length; i++) {
				...
				if (hl6.HL6_STATUS_DETAIL_ID != HL6_STATUS.DELETED_IN_CRM) {
					if (parseFloat(objHl5.BUDGET) < total
							+ parseFloat(hl6.HL6_BUDGET)) {
...
function massSetHl6Status(hl6Ids, userId) {
	...
	var hl6IdsInCrm = [];
	hl6Ids
			.forEach(function(hl6) {
				var status_id = (Number(dataHl6.getHl6StatusByHl6Id(hl6.hl6_id).HL6_STATUS_DETAIL_ID) === HL6_STATUS.DELETION_REQUEST) ? HL6_STATUS.DELETED_IN_CRM
						: !!Number(budgetSpendRequest
								.countPendingBudgetRequestByHl6Id(hl6.hl6_id)) ? HL6_STATUS.IN_CRM_NEED_NEW_BUDGET_APPROVAL
								: HL6_STATUS.IN_CRM;
				hl6List.push({
...
function massResetHl6CategoryOptionUpdated(hl6Id, userId) {
	dataCategoryOptionLevel.massResetHl6CategoryOptionUpdated(hl6Id,
			LEVEL_STRING.toLowerCase(), userId);
	dataHierarchyCategoryCountry.massResetCountryCategoryOptionUpdated(hl6Id,
			LEVEL_STRING.toLowerCase(), userId);
	return true;
}

function resetHl6CategoryOptionUpdated(hl6Id, userId) {
	return dataCategoryOptionLevel.resetHl4CategoryOptionUpdated(hl6Id,
			LEVEL_STRING.toLowerCase(), userId);
}
...
function changeStatusOnDemand(hl6_id, userId, cancelConfirmation, isLegacy) {
	...
	if (hl6.hl6_crm_description == L6_COPY_CRM_DESCRIPTION
			|| hl6.hl6_crm_description == CREATED_FROM_EVENT_REQUEST_CRM_DESCRIPTION) {
		throw ErrorLib.getErrors().CustomError("", "",
				L6_MSG_COULDNT_CHANGE_STATUS_INVALID_CRM_DESCRIPTION);
	}
	...
	if (hl6.HL6_STATUS_DETAIL_ID != HL6_STATUS.IN_CRM
		...
		if (!cancelConfirmation) {
			if (hl6.HL6_STATUS_DETAIL_ID == HL6_STATUS.VALID_FOR_CRM
				...
				if (!existInCrm) {
					...
					statusId = numChanges > 0 ? HL6_STATUS.UPDATE_IN_CRM
							: HL6_STATUS.IN_CRM;
					// If exist in CRM and have changes in CRM fields - set
					// status: UPDATE_IN_CRM
					// If exist in CRM and not have changes in CRM fields - set
					// status: IN_CRM
				}
			...
			statusId = numChanges > 0 ? HL6_STATUS.VALID_FOR_CRM
					: HL6_STATUS.IN_CRM;
		}
		...
		var hl5 = !Number(data.HL5_ID) ? dataHl5
				.getHl5LegacyById(data.HL5_LEGACY_ID) : dataHl5
				.getHl5ById(data.HL5_ID);
		var hl5Id = data.HL5_ID || data.HL5_LEGACY_ID;
		var l4Id = data.HL4_ID || hl5.HL4_ID;
...
function crmFieldsHaveChanged(data, isComplete, userId, isNew, hl6Db) {
	...
	var crmBindingFields = {
		hl6 : Object.keys(deReportDisplayName)
	};
	...
	if (!data.HL6_ID) {
		...
		var oldHl6 = hl6Db ? hl6Db : dataHl6.getHL6VersionedById(data.HL6_ID);
		oldHl6 = oldHl6 ? oldHl6 : {};
		var l6CrmBindigFields = util.getMapHl6ChangedFieldsByHl6Id(data.HL6_ID);

		Object
				.keys(crmBindingFields)
				.forEach(
						function(object) {
							crmBindingFields[object]
									.forEach(function(field) {
										...
										if (Number(oldHl6.HL5_ID) && field === "PARENT_PATH") {
											oldParentPath =
												dataPath.getCrmParentPathByIdLevelId(LEVEL_STRING.toLowerCase(), data.HL6_ID)[0].PARENT_PATH;

											parentPaths = !oldHl6.HL5_LEGACY_ID
												? pathBL.getPathByLevelParentForCrm(LEVEL_STRING.toLowerCase(), data.HL5_ID)
												: {
													deprecatedParentPath: '',
										...
										if (field.indexOf('_DATE') <= 0) {
											...
											case "HL6_CRM_DESCRIPTION":
												...
												break;
											case "BUDGET":
												// When the HL5 is "In CRM", the
												// budget can not change de
												// status, but we need to
												// validate it later to show the
												// correct message.
												...
												break;
											case "PARENT_PATH":
												// If have HL5_ID undefined is
												// HL6_LEGACY and Path dont
												// change
												fieldChanged = !!data.HL5_ID
														&& oldHl6[field] != data[field];
												break;
											default:
										...
										if (fieldChanged
												|| pathChanged
												|| (field == "BUDGET" && budgetChanged)) {
											if (Number(oldHl6.HL5_ID)
													&& field == "PARENT_PATH") {
												if (oldParentPath) {
													if (pathChanged) {
														pathBL
											...
											var in_hl6_crm_binding_id = l6CrmBindigFields[field] || null;

											if (in_hl6_crm_binding_id) {
												parameters.in_hl6_crm_binding_id = in_hl6_crm_binding_id;
												crmBindingChangedFieldsUpdate
														.push(parameters);
											} else {
												crmBindingChangedFields
														.push(parameters);
											}
											if (field !== "BUDGET"
													|| fieldChanged
...
function insertCategoryOption(data, userId) {
	...
	var mapCCOL = util.getMapCountryCategoryOption(LEVEL_STRING.toLowerCase(),
			data.HL2_ID);
	...
	data.CATEGORIES
			.forEach(function(category) {
				category.OPTIONS
						.forEach(function(option) {
							var categoryOptionLevelId = Number(category.CATEGORY_TYPE_ID) == CATEGORY_TYPE.COUNTRY ? mapCCOL[category.CATEGORY_ID][option.OPTION_ID]
	...
	if (categoryOptionBulk.length) {
		dataCategoryOptionLevel.insertCategoryOption(categoryOptionBulk,
				LEVEL_STRING.toLowerCase());
	}
	if (countryCategoryOptionBulk.length) {
		dataCategoryOptionLevel.insertCountryCategoryOption(
				countryCategoryOptionBulk, LEVEL_STRING.toLowerCase());
	}
...
function updateCategoryOption(data, userId, fromChangeStatusOnDemand) {
	...
	data.CATEGORIES
			.forEach(function(category) {
				category.OPTIONS
						.forEach(function(option) {
							var categoryOptionLevelId = category.CATEGORY_TYPE_ID == CATEGORY_TYPE.COUNTRY ? mapCCOL[category.CATEGORY_ID][option.OPTION_ID]
									: mapCOL[category.CATEGORY_ID][option.OPTION_ID];
							...
							var categoryOption = {
								...
								in_amount_kpi : Number(option.AMOUNT_KPI) || 0,
								in_user_id : userId,
								in_updated : fromChangeStatusOnDemand
										&& !!Number(option.AMOUNT) ? 1
										: (option.UPDATED || 0),
								in_hl6_id : data.HL6_ID
							};
	...
	if (updateBulk.length) {
		dataCategoryOptionLevel.updateCategoryOption(updateBulk, LEVEL_STRING
				.toLowerCase());
	}

	if (insertBulk.length) {
		dataCategoryOptionLevel.insertCategoryOption(insertBulk, LEVEL_STRING
				.toLowerCase());
	}

	if (updateCountryCategoryBulk.length) {
		dataCategoryOptionLevel.updateCountryCategoryOption(
				updateCountryCategoryBulk, LEVEL_STRING.toLowerCase());
	}

	if (insertCountryCategoryBulk.length) {
		dataCategoryOptionLevel.insertCountryCategoryOption(
				insertCountryCategoryBulk, LEVEL_STRING.toLowerCase());
	}
...
function insertInternalCofunding(data, automaticBudgetApproval, userId) {
	...
	if (data.SALES && data.SALES.length) {
		var internalCoFundingCurrency = dataCurrency
				.getCurrencyValueId(data.SALE_CURRENCY_ID);
		var arrSaleHl6 = [];
		data.SALES
					...
					if (!aux[sale.ORGANIZATION_ID]) {
						arrSaleHl6
								.push({
									...
									in_description : ORGANIZATION_TYPE[sale.ORGANIZATION_TYPE] === 3 ? sale.DESCRIPTION == '' ? sale.DESCRIPTION
											: 'OTHER'
											: null,
		...
		if (data.CO_FUNDED && data.SALE_REQUESTS && data.SALE_REQUESTS.length)
			budgetSpendRequest.insertSalesBudgetSpendRequest(
					data.SALE_REQUESTS, data.HL6_ID, LEVEL_STRING,
					internalCoFundingCurrency, automaticBudgetApproval, userId);
	}
}
...
function updateInternalCofunding(data, automaticBudgetApproval, userId) {
	if (data.SALES && data.SALES.length) {
						...
						arrSaleHl6
								.push({
									in_hl6_sale_id : sale.HL6_SALE_ID
											|| sale.SALES_ID,
									in_description : ORGANIZATION_TYPE[sale.ORGANIZATION_TYPE] === 3 ? sale.DESCRIPTION
											: null,
									in_currency_id : data.SALE_CURRENCY_ID,
									in_user_id : userId
								});
		...
		data.SALE_REQUESTS.forEach(function(sr) {
			sr.HL_SALES_ID = findHLSalesId(data.SALES, sr.ORGANIZATION_ID,
					sr.ORGANIZATION_TYPE);
		});

		if (data.SALE_REQUESTS && data.SALE_REQUESTS.length)
			budgetSpendRequest.updateSalesBudgetSpendRequest(
					data.SALE_REQUESTS, data.HL6_ID, LEVEL_STRING,
					internalCoFundingCurrency, automaticBudgetApproval, userId);
	}

	if (data.REMOVED_SALES_IDS && data.REMOVED_SALES_IDS.length) {
		var saleBudgetSpendRquestToDelete = data.REMOVED_SALES_IDS
				.map(function(id) {
					return {
						in_budget_spend_request_id : id,
						in_user_id : userId
					};
				});
		budgetSpendRequest.deleteBudgetSpendRequestBySale(
				saleBudgetSpendRquestToDelete, LEVEL_STRING);
	}
	return true;
}
...
function insertExternalCoFunding(data, automaticBudgetApproval, userId) {
	if (data.CO_FUNDED && data.PARTNERS && data.PARTNERS.length) {
		var arrPartner = [];
		var externalCoFundingCurrency = dataCurrency
				.getCurrencyValueId(data.PARTNER_CURRENCY_ID);
		data.PARTNERS
				.forEach(function(partner) {
					var budgetSpendRequestId = budgetSpendRequest
							.insertPartnerBudgetSpendRequest(partner.AMOUNT,
									partner.MESSAGE, data.HL6_ID, LEVEL_STRING,
									externalCoFundingCurrency,
									automaticBudgetApproval, userId);
					arrPartner
							.push({
								in_hl6_id : data.HL6_ID,
								...
								in_intel_project_id : PARTNER_TYPE.INTEL == partner.PARTNER_TYPE_ID  && partner.INTEL_PROJECT_ID ? partner.INTEL_PROJECT_ID
										: null,
								in_claim_id : PARTNER_TYPE.INTEL == partner.PARTNER_TYPE_ID
										&& partner.CLAIM_ID ? partner.CLAIM_ID
										: null,
								in_comments : PARTNER_TYPE.INTEL == partner.PARTNER_TYPE_ID
										&& partner.COMMENTS ? partner.COMMENTS
										: null,
								in_company_name : PARTNER_TYPE.EXTERNAL_PARTNER == partner.PARTNER_TYPE_ID
										&& partner.COMPANY_NAME ? partner.COMPANY_NAME
										: null,
								in_company_address : PARTNER_TYPE.EXTERNAL_PARTNER == partner.PARTNER_TYPE_ID
										&& partner.COMPANY_ADDRESS ? partner.COMPANY_ADDRESS
										: null,
								in_invoice_number : PARTNER_TYPE.EXTERNAL_PARTNER == partner.PARTNER_TYPE_ID
										&& partner.INVOICE_NUMBER ? partner.INVOICE_NUMBER
										: null
							});
				});
...
function updateExternalCoFunding(data, automaticBudgetApproval, userId) {
	if (data.PARTNERS && data.PARTNERS.length) {
		...
		data.PARTNERS
				.forEach(function(partner) {
					if (!partner.PARTNER_ID) {
						var budgetSpendRequestId = budgetSpendRequest
								.insertPartnerBudgetSpendRequest(
										partner.AMOUNT, partner.MESSAGE,
										data.HL6_ID, LEVEL_STRING,
										externalCoFundingCurrency,
										automaticBudgetApproval, userId);
						arrPartnerToInsert
								.push({
					...
					if (partner.ATTACHMENTS && partner.BUDGET_SPEND_REQUEST_ID) {
						partner.ATTACHMENTS = partner.attachments
								|| partner.ATTACHMENTS;
						partnerLib.updateAttachmentPartner(partner,
								LEVEL_STRING, userId);
					}
				});
		...
		if (budgetSpendRequestToUpdate.length)
			budgetSpendRequest.updatePartnerBudgetSpendRequest(
					budgetSpendRequestToUpdate, data.HL6_ID, LEVEL_STRING,
					automaticBudgetApproval, userId);
	}
	...
	if (data.REMOVED_PARTNER_IDS && data.REMOVED_PARTNER_IDS.length) {
		...
		var pendingPartner = dataPartner.getPendingPartnerByPartnerId(
				arrPartnerToDelete, "HL5");
		arrPartnerToDelete = [];
		pendingPartner.forEach(function(elem) {
			arrPartnerToDelete.push({
				in_partner_id : elem.IN_PARTNER_ID,
				in_user_id : userId
			});
		...
		if (arrPartnerToDelete && arrPartnerToDelete.length) {
			dataPartner.deleteHlPartnerByPartnerId(arrPartnerToDelete,
					LEVEL_STRING);
			budgetSpendRequest
					.deleteBudgetSpendRequest(arrBudgetSpendRequestToDelete);
		}
	}
	return true;
}
...
function validateKpi(data) {
	if (data.TARGET_KPIS && !Number(data.FORECAST_AT_L5)) {
		if (!data.TARGET_KPIS.KPIS.length && !data.TARGET_KPIS.COMMENTS)
			throw ErrorLib.getErrors().CustomError("",
					"hl6Services/handlePost/insertHl6",
					L6_CAMPAIGN_FORECASTING_KPIS_COMMENT);

		data.TARGET_KPIS.KPIS.forEach(function(hl6ExpectedOutcomesDetail) {
			if (hl6ExpectedOutcomesDetail.VOLUME_VALUE != 0
					&& !Number(hl6ExpectedOutcomesDetail.VOLUME_VALUE))
				throw ErrorLib.getErrors().CustomError("",
						"hl6Services/handlePost/insertHl6",
						L6_CAMPAIGN_FORECASTING_KPIS_DETAILS);
			if (!hl6ExpectedOutcomesDetail.EURO_VALUE
					|| !Number(hl6ExpectedOutcomesDetail.EURO_VALUE))
				throw ErrorLib.getErrors().CustomError("",
						"hl6Services/handlePost/insertHl6",
						L6_CAMPAIGN_FORECASTING_KPIS_DETAILS_EURO);
			if (!hl6ExpectedOutcomesDetail.OUTCOMES_ID
					|| !Number(hl6ExpectedOutcomesDetail.OUTCOMES_ID))
				throw ErrorLib.getErrors().CustomError("",
						"hl6Services/handlePost/insertHl6",
						L6_CAMPAIGN_FORECASTING_KPIS_NOT_VALID);
		});
	}
	return true;
}
			...
			if (ORGANIZATION_TYPE[sale.ORGANIZATION_TYPE] === 3) {
				if (sale.DESCRIPTION != '' && !sale.DESCRIPTION)
					throw ErrorLib.getErrors().CustomError(
							...
							"",
							levelCampaign
									+ " Sales description can not be found.");
			} else {
				if (!sale.ORGANIZATION_ID || !Number(sale.ORGANIZATION_ID))
					throw ErrorLib.getErrors().CustomError(
							"",
							"",
							levelCampaign + " Sales " + key
									+ " can not be found.");
			}
		});
	}
...
function validateCategoryOption(data) {
	if (!data.CATEGORIES) {
        throw ErrorLib.getErrors().CustomError("", "", L6_CATEGORY_NOT_EMPTY);
    }

	if (!data.HL6_ID && data.CATEGORIES.length !== dataCategory.getAllocationCategoryCountByHlId(LEVEL_STRING.toLowerCase())) {
        throw ErrorLib.getErrors().CustomError("", "", L6_CATEGORY_INCORRECT_NUMBER);
    }
...
function insertExpectedOutcomes(data, userId) {
	var hl6ExpectedOutcomesId = dataExOut.insertHl6ExpectedOutcomes(
			data.HL6_ID, data.TARGET_KPIS.COMMENTS || '', userId);
	var mapEOL = expectedOutcomesLevelLib
			.getKpiTypeOptionMapByHlId(HIERARCHY_LEVEL.HL6);
	if (data.TARGET_KPIS.KPIS.length) {
		var hl6ExpectedOutcomesDetail = [];
		data.TARGET_KPIS.KPIS
				.forEach(function(expectedOutcomeDetail) {
					hl6ExpectedOutcomesDetail
							.push({
...
function getExternalCofunding(hl6Id) {
	...
	var partnerCurrencyValue = (Number(partner.partnerCurrencyValue || 1))
			.toFixed(2);
	return {
		PARTNERS : partner.partners,
		PARTNER_INTEL_TOTAL : (Number(partner.total) || 0).toFixed(2),
		PARTNER_INTEL_TOTAL_EUROS : ((Number(partner.total) || 0) / partnerCurrencyValue)
				.toFixed(2),
		...
		PARTNER_EXTERNAL_TOTAL_EUROS : (Number(partner.totalExternal) / partnerCurrencyValue)
				.toFixed(2)
	};
}

function getCategoryOption(hl6Id, fromCloneMethod) {
    return allocationCategoryOptionLevelLib.getHlCategoryOptionByLevelHlId(LEVEL_STRING, hl6Id, fromCloneMethod);
}

function getCategoryOptionVersioned(hl6Id) {
	return allocationCategoryOptionLevelLib
...
function getTargetAudienceByHl6Id(hl6Id) {
	if (!hl6Id) {
		throw ErrorLib.getErrors().BadRequest("The Parameter ID is not found", "hl6Services/handleGet/getTargetAudienceByHl6Id", L6_MSG_INITIATIVE_NOT_FOUND);
	}
	var targetAudienceArray = dataHl6.getTargetAudienceByHl6Id(hl6Id);
	var result = [];

	if(targetAudienceArray && targetAudienceArray.length){
		targetAudienceArray.forEach( function(target) {
			result.push(target.TARGET_AUDIENCE_ID + ""); //The SAPUI5 Multi-Combobox only accepts string[]. Yes, it sucks.
		});
	}
...
function clone(cloneHl6Id, userId) {
    var data = getHl6ById(cloneHl6Id, true).HL6;
    ...
    data = util.completeFromDynamicForm(data.HL5_ID, "L6", data, false, false, true);
    var hl6Id = insertData(data, acronym);
    if (isLegacy) {
        dataHl6.insertHl6Legacy(hl6Id, hl5Id);
    }
    pathBL.insParentPath(LEVEL_STRING.toLowerCase(), hl6Id, hl5Id, userId, hl5.PATH);
    data.HL6_ID = hl6Id;
    data.HL2_ID = dataHl2.getHl2ByHl4Id(l4Id).HL2_ID;
    insertExpectedOutcomes(data, userId);
    ...
    insertCategoryOption(data, userId);
	insertTargetAudience(hl6Id, data.TARGET_AUDIENCE, userId);
    var CRM_ID = isLegacy ? hl5.PATH + ('' + acronym).toUpperCase() : dataPath.getPathByLevelHlId(LEVEL_STRING.toLowerCase(), hl6Id);
...
function getActionPermission(hl6, userId, isSuperAdmin) {
	...
	var enableEdition = util.getEnableEdit(hl6.STATUS_ID
			|| hl6.STATUS_DETAIL_ID, HL6_STATUS, userId, isSuperAdmin,
			hl6.PARENT_STATUS_ID, hl6.GRAN_PARENT_STATUS_ID);
...
function insertInCrmVersion(hl6Id) {
	...
	// Save the version of CategoryOptions IN_CRM o UPDATE_IN_CRM to compare
	// later
	dataCategoryOptionLevel.insertCategoryOptionVersioned("HL6", hl6Id,	hl6VersionedId);
	// Save the version of Country-CategoryOptions IN_CRM o UPDATE_IN_CRM to
	// compare later
	return dataCategoryOptionLevel.insertCountryCategoryOptionVersioned("HL6",
			hl6Id, hl6VersionedId);
}

function updEnableCrmCreation(hl6Id, flag) {
	return dataHl6.updEnableCrmCreation(hl6Id, flag);
}
