...
function insertHl6(data, userId) {
    ...
    if (data.STATUS_DETAIL_ID > 0) {
        ...
        if (hl6_id > 0) {
            ...
            var automaticBudgetApproval = blLevel2.getHl2AllowAutomaticBudgetApprovalByHl4Id(l4Id) && data.IN_BUDGET && !!Number(data.BUDGET);
...
function updateHl6(data, userId) {
    ...
    if (data.STATUS_DETAIL_ID > 0) {
        ...
        var automaticBudgetApproval = blLevel2.getHl2AllowAutomaticBudgetApprovalByHl5Id(data.HL5_ID) && data.IN_BUDGET && !!Number(data.BUDGET);
...
function isComplete(data) {
    ...
    for (var i = 0; i < crmBindingFields.length; i++) {
            ...
            case 'CATEGORY':
                ...
                if (isComplete) {
                    for (var j = 0; j < data.CATEGORIES.length; j++) {
                        ...
                        if (!isComplete) {
                            throw JSON.stringify(hl6Category);
                            break;
                        }
                    }
                }
        ...
        if (!isComplete) {
            throw JSON.stringify({key: crmBindingField, data: data});
            break;
        }
    }
    if (isComplete) {
...
function validateHl6(data, userId) {
    ...
    if (isHl6Complete) {
        ...
        if (data.HL6_ID) {
            ...
            if (!crmFieldsHasChanged && !categoryHasChanged
                && !Number(budgetSpendRequest.countPendingBudgetRequestByHl6Id(data.HL6_ID))) {
                if (data.STATUS_DETAIL_ID == HL6_STATUS.IN_PROGRESS) {
...
function setStatusInCRM(hl6_id, userId) {
    ...
    for (var i = 0; i < hl6Ids; i++) {
        ...
        result = null;
        result = setStatus(hl6Ids[i], HL6_STATUS.IN_CRM, userId);
        if (result) {
