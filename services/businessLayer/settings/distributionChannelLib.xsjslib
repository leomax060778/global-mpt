...
function validate(distributionChannel) {
    ...
    dc = dataDistributionChannel.getDistributionChannelByCrmKey(distributionChannel.IN_CRM_KEY);
    if(dc && Number(distributionChannel.IN_DISTRIBUTION_CHANNEL_ID) !== Number(dc.DISTRIBUTION_CHANNEL_ID))
        throw ErrorLib.getErrors().CustomError("", "distributionChannelService/handlePost/validateDistributionChannel", DISTRIBUTION_CHANNEL_CRM_KEY_EXISTS);
