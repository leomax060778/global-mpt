...
function handleGet() {
	...
	try{
		...
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::GET_OPTION_BY_CATEGORY_ID');
...
function handlePost(reqBody) {
	...
	try{
		var query = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_OPTION"(?,?,?,?,?)';
		...
		if(spResult){
			...
			var fnSell = connHdb.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::GET_' + hierarchyLevel[hierarchy_level_id] + '_CATEGORY_BY_CATEGORY_ID');
			...
			query = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_' + hierarchyLevel[hierarchy_level_id] + '_CATEGORY_OPTION"(?,?,?,?,?,?)';
			var changeStatusQuery = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::' + hierarchyLevel[hierarchy_level_id] + '_CHANGE_STATUS"(?,?,?,?)';
			var logChangeStatusQuery = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_' + hierarchyLevel[hierarchy_level_id] + '_LOG_STATUS"(?,?,?,?)';
			var countOptionByCategoryId = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::GET_OPTION_COUNT_BY_CATEGORY_ID"(?,?)';
...
function handleDelete(reqBody) {
	var conn = $.db.getConnection();
	try{
		var query = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::DEL_OPTION"(?,?,?)';
...
function handlePut(reqBody) {
	...
	try{
		...
		var order_option = reqBody.ORDER_OPTION;
		var modified_user_id = reqBody.MODIFIED_USER_ID;
		var query = 'call "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::UPD_OPTION"(?,?,?,?,?,?)';
