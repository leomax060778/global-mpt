...
function getTokenSession(reqBody){
	...
	try {
		...
		var fnUser = conn.loadProcedure('MKTG_PLANNING_TOOL',
				'mktgplanningtool.db.procedures::GET_USER_BY_USERNAME');
...
function getUserToken(userId) {
	...
	try {
		...
		var spUserToken = conn2
				.loadProcedure('MKTG_PLANNING_TOOL',
						'mktgplanningtool.db.procedures::GET_USER_SESSION_TOKEN_BY_USER_ID');
...
function hashPassword(reqBody){
	...
	try {
		...
		// Look for user in the db
		var spHash256 = conn.loadProcedure('MKTG_PLANNING_TOOL',
				'mktgplanningtool.db.procedures::GET_HASH_SHA256');
...
function insertUserRole(){
	...
	try {
		...
		var fn = conn.loadProcedure('MKTG_PLANNING_TOOL',
				'mktgplanningtool.db.procedures::INS_USER_ROLE');
...
function updateUserRole(){
	...
	try {
		...
		var fn = conn.loadProcedure('MKTG_PLANNING_TOOL',
				'mktgplanningtool.db.procedures::UPD_USER_ROLE_BY_USER_ID');
...
function getCMSPath(){
	...
	try {
		...
		var spGetPath = conn2
				.loadProcedure('MKTG_PLANNING_TOOL',
						'mktgplanningtool.db.procedures::GET_PATH_ORGANIZATION_ACRONYM');
