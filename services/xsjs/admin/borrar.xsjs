...
function handlePut()
{
	...
	try {
		...
		var query = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_EURO_CONVERSION"( ?, ?, ?, ?, ?, ?)';
...
function ObtenerDatos(){
	try{
		...
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::GET_ORIGIN_PARENT');
		var result = fnSell(1);
...
function getOriginParent2(){
	...
	try{
		conn = $.db.getConnection();
		//GET_ORIGIN_GLOBE
		var queryGetOriginParent = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::GET_ORIGIN_PARENT"(?,?)';
...
function insertarEuroConversion(){
	...
	try{
		...
		//var insert = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_CRM"(?,?,?,?,?,?,?,?)';
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::INS_EURO_CONVERSION');
...
function insertarEuroConversion(){
	...
	try{
		conn = $.hdb.getConnection();
		//var insert = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_CRM"(?,?,?,?,?,?,?,?)';
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::INS_LOG_ERROR');
		//in_country, in_currency_abbreviation, in_currency_name, in_currency_value_admin, in_user_id, out out_euro_conversion_id
		var parameters = {};
...
function insertarCRM(){
	...
	try{
		conn = $.hdb.getConnection();
		//var insert = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_CRM"(?,?,?,?,?,?,?,?)';
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::INS_CRM');
...
function insertarHL3(){
	...
	try{
		conn = $.hdb.getConnection();
		//var insert = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_CRM"(?,?,?,?,?,?,?,?)';
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::INS_HL3');
...
function getAllPlan(){
	...
	try{
		conn = $.hdb.getConnection();
		//var insert = 'CALL "MKTG_PLANNING_TOOL"."mktgplanningtool.db.procedures::INS_CRM"(?,?,?,?,?,?,?,?)';
		var fnSell = conn.loadProcedure('MKTG_PLANNING_TOOL', 'mktgplanningtool.db.procedures::GET_ALL_PLAN');
		var result = fnSell({});
		var list = result['out_result'];
		var spResult = [];
